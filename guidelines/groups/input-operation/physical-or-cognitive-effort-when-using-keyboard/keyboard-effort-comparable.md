...
- A review has been conducted and changes made (when necessary) to ensure that there is minumal difference between the number of input commands required when using the :term[keyboard interface] only and the number of commands when using other input modalities.
